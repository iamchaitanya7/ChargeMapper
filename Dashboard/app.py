...
# Setting page config
st.set_page_config( page_title="Optimal Placement of EV charging stations",
...
logo_url = "https://img.icons8.com/external-tal-revivo-green-tal-revivo/100/external-power-location-on-map-for-quick-ev-charge-battery-green-tal-revivo.png"
...
st.markdown("Optimal Placement of EV station based on socio-economic features such as points-of-interest, existing EV charging stations, residential, and commercial areas, demand, etc.")

# Declaring icons: 
park_icon = "Data/legend_img/park.png"
...
school_icon = "Data/legend_img/school.png"
...
place_of_worship_icon = "Data/worship.png"
...
with col2: 
    with st.container():
        ...
        st.subheader("Legend")
        image = Image.open('Data/IconList1.png')
...
with col1:
    ...
    #folium.TileLayer('cartodbpositron').add_to(saarbrucken_map_markers)

    folium.Marker(
        location=[49.24015720000001, 6.996932700000002],
        popup="Saarbrucken",
        icon=folium.Icon(color="yellow",icon="glyphicon glyphicon-pushpin"),
    ...
    for lat, lng, parking, park_space, restaurant,residential,school,retail, community_center,place_of_worship,university,cinema,library,commercial,government, evs in zip(df_saarbrucken_poi.latitude, 
        ...
        if parking != 0 and parking_check==True:
            custom_marker = folium.Marker(location=[lat+0.0015, lng], popup=f"Parks: {parking}",opacity=op, icon=folium.CustomIcon(park_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if park_space != 0 and parkingSpaces_check ==True:
            custom_marker = folium.Marker(location=[lat, lng], popup=f"Parking spaces: {park_space}",opacity=op,  icon=folium.CustomIcon(parking_space,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if restaurant != 0 and restaurant_chk:
            custom_marker = folium.Marker(location=[lat, lng+0.0015], popup=f"Restaurant: {restaurant}",opacity=op,  icon=folium.CustomIcon(restaurant_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if residential != 0 and residential_Chk:
            custom_marker = folium.Marker(location=[lat+0.0015, lng+0.0015], popup=f"Residential: {residential}", opacity=op, icon=folium.CustomIcon(residential_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if school != 0 and school_Chk:
                custom_marker = folium.Marker(location=[lat+0.0017, lng+0.0017], popup=f"Schools: {school}", opacity=op, icon=folium.CustomIcon(school_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if retail != 0 and retail_Chk:
                custom_marker = folium.Marker(location=[lat+0.0017, lng], popup=f"Retail: {retail}", opacity=op, icon=folium.CustomIcon(retail_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if community_center != 0 and community_center_Chk:
                custom_marker = folium.Marker(location=[lat, lng+0.0017], popup=f"Community center: {community_center}", opacity=op, icon=folium.CustomIcon(community_center_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if place_of_worship != 0 and place_of_worship_Chk:
                custom_marker = folium.Marker(location=[lat, lng+0.0017], popup=f"Place of worship: {place_of_worship}", opacity=op, icon=folium.CustomIcon(place_of_worship_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if university != 0 and university_Chk:
                custom_marker = folium.Marker(location=[lat+0.0017, lng], popup=f"University: {university}", opacity=op, icon=folium.CustomIcon(university_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if cinema != 0 and cinema_Chk:
                custom_marker = folium.Marker(location=[lat+0.0017, lng], popup=f"Cinema: {cinema}", opacity=op, icon=folium.CustomIcon(cinema_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if library != 0 and library_Chk:
                custom_marker = folium.Marker(location=[lat+0.0010, lng], popup=f"Library: {library}", opacity=op, icon=folium.CustomIcon(library_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if commercial != 0 and commercial_Chk:
                custom_marker = folium.Marker(location=[lat, lng+0.0010], popup=f"Commercial: {commercial}", opacity=op, icon=folium.CustomIcon(commercial_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
        if government != 0 and government_Chk:
                custom_marker = folium.Marker(location=[lat+0.0010, lng+0.0010], popup=f"Government: {government}", opacity=op, icon=folium.CustomIcon(government_icon,icon_size=(20,20), popup_anchor=(0, -22))).add_to(saarbrucken_map_markers)
    ...
    pred_icon_cross="Data/charging-station.png"
    ...
    saarbrucken_map_markers.save("MymapNew.html")
